...
// echo $srno;
include('con.php');
$sql = $sql="SELECT * FROM `productsdet` WHERE srno=$srno";
$result=mysqli_query($con,$sql);
$row = mysqli_fetch_assoc($result);
$cnameerr="";
?>

    <form action="5updatecat.php" method="POST">
        <label>
            <h1>Category Name</h1>
            <input type="hidden" name="srno" value="<?php echo $row['srno']; ?>">
            <input id="cname" name="cname" type="text" value="<?php echo $row['cname']; ?>"/>
            <span class="error"> <?php echo $cnameerr; ?> </span>
        </label><br> <br>        
        <button type="submit" name="submit" method="POST">update</button>
    </form>
